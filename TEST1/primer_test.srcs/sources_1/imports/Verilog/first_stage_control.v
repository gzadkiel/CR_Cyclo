module first_stage_control # (
    parameter P       = 256 , //! Number of data blocks
    ...
    output                     o_m_axis_data_tready         , //! Asserted by the external slave to signal that it is ready to accept data
...
// Localparam
localparam ADDR_WIDTH   = $clog2(P)  ; //! Number of bits used for the block Address
...
reg  [$clog2(P)             - 1 : 0] r_write_addr     ; //! Register that stores the current RAM Address to write 
reg  [$clog2(P)             - 1 : 0] r_read_addr0     ; //! Register that stores the current RAM Address to read
reg  [2*NP - 1 : 0][NB_DATA - 1 : 0] r_fft_buffer     ; //! Buffer that stores the current data block being fed to the FFT module
reg  [$clog2(NP)            - 1 : 0] r_index          ; //! Index to feed samples from Buffer to FFT Module
...
// --------------------------------------------------------------------------------------
always @(i_NFFT_sel) begin
    case (i_NFFT_sel) // Select FFT Points 
        3'b000 : assign NFFT = 8'b00000100; // NP = 16
        3'b001 : assign NFFT = 8'b00000101; // NP = 32
        3'b010 : assign NFFT = 8'b00000110; // NP = 64
        3'b011 : assign NFFT = 8'b00000111; // NP = 128
        3'b100 : assign NFFT = 8'b00001000; // NP = 256
        3'b101 : assign NFFT = 8'b00001001; // NP = 512
        3'b110 : assign NFFT = 8'b00001010; // NP = 1024
        default: assign NFFT = 8'b00001010; // NP = 1024
    endcase
end

assign CP_LEN  = {1'b0,i_CP_LEN_sel}; // Add padding to input CP_LEN
