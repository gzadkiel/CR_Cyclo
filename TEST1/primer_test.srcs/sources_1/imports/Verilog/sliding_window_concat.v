//! Concatenates outputs from input buffers to be stored in a single RAM module.

module sliding_window_concat #(
    parameter NB_INPUT       = 16       , //! Number of bits of each data sample
    parameter NB_WINFRAME    = 1024 * 10, //! Number of bits of the entire window coefficient array
    parameter NB_CONCAT_PROD = 1024 * 16, //! Number of bits of the entire output product array
    parameter NP_FFT         = 1024       //! Number of samples per data block
) (
...
localparam NB_SAMPLES  = NB_INPUT       ; //! Number of bits of each data sample
...
localparam NB_WIND     = 10             ; //! Number of bits of each window coeff
...
always @(posedge clock) begin: Concat_SWindow_Prods
    if (w_ovalid_im == 1'b1 && w_ovalid_re == 1'b1) begin
        ...
        r_concat_products <= {NB_CONCAT_PROD{1'bZ}};
        r_valid_frame     <= 1'b0;
    end
end
